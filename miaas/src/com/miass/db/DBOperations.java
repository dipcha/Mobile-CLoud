...
public class DBOperations {
    ...
    public static void launchEmulator(String id)
    {
        ...
        try
        {
            String name=null,port=null,target=null;
            ResultSet rs=connect.createStatement().executeQuery("SELECT name,port,version FROM emulator where idemulator='"+id+"'");
            if(rs.next())
            { name=rs.getString(1);
            port=rs.getString(2);
            ...
            if(name!=null)
            {
                URL url=new URL("http://localhost:9090/MyTacks/cloudMachine/create?emulName="+name+"&portNo="+port+"&targetId="+target);
            HttpURLConnection connection=(HttpURLConnection)url.openConnection();
            System.out.println("http://localhost:9090/MyTacks/cloudMachine/create?emulName="+name+"&portNo="+port+"&targetId="+target);
            connection.setRequestMethod("GET");
            connection.connect();
            System.out.println(connection.getResponseCode());
     ...
     public static void stopEmulator(String id)
    {
        ...
        try
        {
            ...
            if(name!=null)
            {
                URL url=new URL("http://localhost:9090/MyTacks/cloudMachine/Stop?name="+name);
            HttpURLConnection connection=(HttpURLConnection)url.openConnection();
